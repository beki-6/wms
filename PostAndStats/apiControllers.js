...
const updatePost = async (req, res) => {
  ...
  if (req.body.dateModified != null) {
    res.post.dateModified = req.body.dateModified;
  }
  try {
...
const updateStat = async (req, res) => {
  if (req.body.category != null) {
    res.stat.category = req.body.category;
  }
  if (req.body.date != null) {
    res.stat.date = req.body.date;
  }
  if (req.body.totalNumberOfIssuesInCategory != null) {
    res.stat.totalNumberOfIssuesInCategory =
      req.body.totalNumberOfIssuesInCategory;
  }
  try {
...
async function getPostById(req, res, next) {
  ...
  try {
    ...
    if (post == null) {
      res
        .status(404)
        .json({ message: "Post with the specified Id doesn't exist." });
    }
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
async function getStatById(req, res, next) {
  ...
  try {
    ...
    if (stat == null) {
      res
        .status(404)
        .json({ message: "Stat with the specified Id doesn't exist." });
    }
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
